...
class Opt_call_hist(Base):
    __tablename__ = 'opt_call_hist'
...
filenames = glob.glob('Resources/opt_calls/*.csv')

for filename in filenames:
    file_name = f"{filename}"
    df = pd.read_csv(file_name)
    df.to_sql(name=Opt_call_hist.__tablename__, con=db, if_exists='append', index=False)
...
results = s.query(Opt_call_hist).all()
for r in results:
    print(r)
